...
const TeacherAccount = () => {
  const { teachers, teacherId, currentTeacher, setCurrentTeacher } = useContext(LoginContext)
  console.log(teacherId)
  const navigate = useNavigate()
  const [edit, setEdit] = useState(true)
  const [info, setInfo] = useState({
    email: currentTeacher && currentTeacher.email,
    user: currentTeacher && currentTeacher.user,
  })
  ...
  const handleSave = async () => {
    await axios.post(`${api_base}/teacher/${teacherId}/edit`, info)
    setCurrentTeacher({
      ...currentTeacher,
      ...info
    })
